  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[24px] overflow-hidden border border-[#E9DDCB] relative">
        <img src={IMG_OFFICE} alt="Bureau RH" className="absolute inset-0 w-full h-full object-cover"/>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {[
          ...
          <div key={c.label} className="rounded-[20px] bg-white border border-[#E9DDCB] p-5 flex items-center gap-4">
            <div className="h-10 w-10 rounded-xl bg-[#1B1D3B] text-[#C9A86A] flex items-center justify-center"><c.icon size={18}/></div>
      ...
      <div className="rounded-[24px] bg-[#1B1D3B] text-white p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3"><div className="h-10 w-10 rounded-xl bg-white/10 flex items-center justify-center"><Users size={18}/></div><div><div className="font-bold">Équipe RH</div><div className="text-sm text-white/70">3 sièges inclus • SSO & journaux d'audit</div></div></div>
        <button onClick={()=>showToast('Invitation envoyée')} className="rounded-full bg-[#C9A86A] text-[#1B1D3B] px-5 py-2.5 text-sm font-bold">Inviter un collaborateur</button>
      </div>
    </div>
  )
}
